 ...
 *
 * Testability: everything that touches the outside world (child_process, the
 * native module, process.env, process.platform) arrives through
 * `createWallpaperMode(deps)`, and all mutable state (native load cache, attach
 * state, op chain, the env-derived E2E disable flag) lives on the instance. The
 * default export is a singleton built from the real deps with the same method
 * names as before, so callers are unchanged.
 */
const { execFile: realExecFile } = require('child_process')
const fs = require('fs')
const path = require('path')

function createWallpaperMode(deps = {}) {
  const {
    execFile = realExecFile,
    // Separate function so a test can hand back a stub without
    // electron-as-wallpaper being installed/buildable.
    loadNativeModule = () => require('electron-as-wallpaper'),
    env = process.env,
    platform = process.platform,
    resourcesPath = process.resourcesPath,
    // Fallback log sink when a caller doesn't pass one per call.
    logger = null
  } = deps

  const e2eDisable = env.APIA_E2E_DISABLE_WALLPAPER === '1'
  ...
  function loadNative() {
    ...
    if (platform !== 'win32') {
      cachedLoadError = new Error(`wallpaper mode is Windows-only (platform=${platform})`)
      return null
    }
    if (e2eDisable) {
  ...
  // source, committed + shipped via extraResources. (An earlier .ps1 fallback
  // was dropped: it lacked the z-order fix, so if it ever ran it would
  // reproduce the "hidden behind the wallpaper" bug.)
  //
  ...
  function resolveHelper() {
    const exe = firstExisting(
      resourcesPath ? path.join(resourcesPath, 'scripts', 'win-wallpaper.exe') : null,
  ...
  async function tryProgmanChild(window, log) {
    if (platform !== 'win32' || e2eDisable) return false
   ...
   */
  function enableWallpaper(window, log = logger) {
   ...
   */
  function disableWallpaper(window, log = logger) {
    return serialize(async () => {
...
module.exports = { ...createWallpaperMode(), createWallpaperMode }
